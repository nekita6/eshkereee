...
goGameBtn.addEventListener('click', function () {
	...
	sectionGameMenu.style.display = 'none';
...
function draw() {
	...
	if (scoreboard.isGameOver()) {
		document.querySelector('#sectionGameMenu p').innerHTML ='для управления используй стрелки на клавиатуре<br>у тебя осталось <mark>'+lives+'</mark> попыток';
		if (lives==0) {
			document.querySelector('#sectionGameMenu p').innerHTML ='нужно поучиться ещё, чтобы продолжить играть';
			sectionGameMenu.innerHTML += '<button onclick="closeTest()">выбрать другую группу инструментов</button>';
			goGameBtn.style.display = 'none';
		}
		sectionGameMenu.style.display = 'block';
		sectionGame.style.display = 'none';
		if(scoreboard.getTopscore() > localStorage.getItem('topScore')) {
			localStorage.setItem('topScore', scoreboard.getTopscore());
			statusUpdate();
		}
	} else {
		drawFallingShape();
...
function drawUI() {
	...
	g.font = smallFont;
	g.fillText('рекорд       ' + scoreboard.getTopscore(), scoreX, scoreY);
	g.fillText('уровень      ' + scoreboard.getLevel(), scoreX, scoreY + 30);
	g.fillText('линии        ' + scoreboard.getLines(), scoreX, scoreY + 60);
	g.fillText('счёт         ' + scoreboard.getScore(), scoreX, scoreY + 90);
